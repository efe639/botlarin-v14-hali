...
const { panels } = require("../../../settings.js");
let sended = false;
setInterval(() => {
...
module.exports = async (message) => {
    ...
    const embed = new EmbedBuilder().setColor('#DD0909').setTimestamp().setAuthor({ name: message.member.displayName, iconURL: message.author.avatarURL({ dynamic: true }) }).setFooter({ text: "Ktria Vortex", iconURL: message.guild.iconURL({ dynamic: true}) })
